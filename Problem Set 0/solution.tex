...
\usepackage{amsmath, amssymb, amsfonts, mathtools, fancyhdr, tikz, cancel, enumitem, lastpage, listings}
...
\begin{itemize}
	...
	\item{\textcolor{for}{\textbf{Problem 6}}}
		\begin{enumerate}[label=(\alph*)]
			...
			\item {\textbf{Make $G/\ker{(\phi)}$ to a group.}}\\
				Assume that $(\ker{\phi}:=N)$ which is a normal subgroup of $G$ by the previous parts, we need to define a multiplication
				on $G/N$, let's define it as follows:
				\begin{align*}
					&(Ng) (Nh) = N(gh)  &&(g,h\in G)
				\end{align*}
				Suppose that we have different representives for the same cosets, i.e. $Ng = Ng'$ and $Nh = Nh'$
				We need to show that $N(gh) = N(g'h')$
				\begin{align*}
					\left\{\begin{array}{ll}
						Ng = Ng' \rightarrow& g(g')^{-1}=n_1 \in N \\
						Nh = Nh' \rightarrow& h(h')^{-1}=n_2 \in N
					\end{array}\right. \Longrightarrow
					(gh)(g'h')^{-1} = (gh)(h')^{-1}(g')^{-1}	
					\xrightarrow{N\unlhd G}\ \in N
				\end{align*}
				Thus $N(gh) = N(g'h')$ which means that our multiplication is well-defined. This multiplication make our set to a group
				is straightforward by definition.
		\end{enumerate}
	\item{\textcolor{for}{\textbf{Problem 7}}}
		\begin{enumerate}[label=(\alph*)]
			\item Code is provided in a separated file.
			\item Code is provided in a separated file.
		\end{enumerate}
